package gamerpg.worlds 
{
	import flash.utils.getDefinitionByName;
	import fputils.worlds.RPGWorld;
	import gamerpg.entities.FinishArea;
	import gamerpg.entities.Star;
	import gamerpg.entities.ZeldaPlayer;
	 ...
	 */
	public class GameWorldRPG extends RPGWorld 
	{
		...
		public function GameWorldRPG(mapData:Class = null) 
		{
			super();
			...
			else
			{
				...
				player = new ZeldaPlayer(640, 640, 0, GData.playerSpeed, GData.gridWidth, GData.gridHeight ) as Entity;
				player.type = "player";
		...
		protected function updateFailureState():void
		{
			...
			{
				FP.world = new GameWorldRPG(_mapData);
			}
		}
		...
		protected function loadMap(mapData:Class):void
		{
			...
			// Create our map grid.
			_mapGrid = new Grid(uint(mapXML.@width), uint(mapXML.@height), GData.gridWidth, GData.gridHeight, 0, 0);
			_mapGrid.loadFromString(String(mapXML.Grid), "", "\n");
			
			// Create a player at the player start.
			player = new ZeldaPlayer(int(mapXML.Entities.PlayerStart.@x), int(mapXML.Entities.PlayerStart.@y), int(mapXML.Entities.PlayerStart.@angle), GData.playerSpeed, GData.gridWidth, GData.gridHeight ) as Entity;
			player.type = "player";
